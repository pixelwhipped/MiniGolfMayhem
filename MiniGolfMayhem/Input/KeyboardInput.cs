...
namespace MiniGolfMayhem.Input
{
    ...
    {
        public KeyboardInput(Golf game)
            : this(game, Color.White, .75f, false)
        {
        }

        public KeyboardInput(Golf game, Color oskColor, float oskAlpha, bool visible)
        {
            Game = game;
        ...
        public void Draw(SpriteBatch batch)
        {
            ...
            {
                batch.Draw(_oskUpper, _oskOffset, null, Color.White*OskAlpha);
                var r = new Rectangle((int) (_oskOffset.X + _oskCapsRect.X), (int) (_oskOffset.Y + _oskCapsRect.Y),
                    _oskCapsRect.Width,
                    _oskCapsRect.Height);
                batch.Draw(Golf.Pixel, r, null, Branding.ForegroundColor*.5f);
            }
            else
            {
            ...
            foreach (var b in _oskButtonHits)
            {
                var r = new Rectangle((int) (_oskOffset.X + b.Rect.X), (int) (_oskOffset.Y + b.Rect.Y), b.Rect.Width,
                    b.Rect.Height);
                batch.Draw(Golf.Pixel, r, null, Branding.ForegroundColor*b.TimeToLive*.5f);
            }
            batch.End();
        }
        ...
        public void AddKeyboardListener(Procedure<KeyboardInput> listener) => _keyboardListeners.Add(listener);
        ...
        public bool IsOskVisable
        {
            ...
            get { return _oskVisible && _oskShowHide.IsComplete; }
        }
        ...
        public bool TypedKey(Keys k) => !_oskCurrentState.Contains(k) && _oskPreviousState.Contains(k);

        public bool Pressed(Keys k) => _oskCurrentState.Contains(k);

        public bool Released(Keys k) => !Pressed(k);

        public bool Any() => Enum.GetValues(typeof (Keys)).Cast<Keys>().Any(TypedKey);

        public IEnumerable<Keys> TypedKeys => Enum.GetValues(typeof (Keys)).Cast<Keys>().Where(TypedKey);

        public bool AnyAlpha() => _oskCurrentState.Any(k => _alphaKeys.Contains(k) && !_oskPreviousState.Contains(k));

        public bool AnyNumeric() => _oskCurrentState.Any(k => _numericKeys.Contains(k) && !_oskPreviousState.Contains(k));

        public bool AnyAlphaNumeric() => AnyNumeric() || AnyAlpha();
